...
void Q3DViewerExtension::applyClut(const TransferFunction &clut, bool preset)
{
	...
	//
	//VoiLut voiLut(clut);
	//double w = lut.getWindowLevel().getWidth();
	//double c = lut.getWindowLevel().getCenter();
	//VoiLut voiLut = WindowLevel(6263, 4326);
	//voiLut.setWindowLevel(WindowLevel(6263, 4326, clut.name()));
	//m_3DView->setTransferFunction(voiLut.getLut());
    emit newTransferFunction();
//     this->render();
	updateAnnotations();
}
